...
//An optional abstract base class for commands
public abstract class CommandBase extends BukkitCommand implements CommandExecutor {
    ...
    private int delay = 0; //Cooldown
    private final int minArguments;
    private final int maxArguments;
    private final boolean playerOnly;

    public CommandBase(String command, String description) {
        this(command, description, 0);
    }

    public CommandBase(String command, String description, boolean playerOnly) {
        this(command, description, 0, playerOnly);
    }

    public CommandBase(String command, String description, int requiredArguments) {
        this(command, description, requiredArguments, requiredArguments);
    }

    public CommandBase(String command, String description, int minArguments, int maxArguments) {
        this(command, description, minArguments, maxArguments, false);
    }

    public CommandBase(String command, String description, int requiredArguments, boolean playerOnly) {
        this(command, description, requiredArguments, requiredArguments, playerOnly);
    }

    public CommandBase(String command, String description, int minArguments, int maxArguments, boolean playerOnly) {
        super(command);
    ...
    public boolean execute(CommandSender sender, String alias, String[] arguments) {
        ...
        if(delayedPlayers != null && sender instanceof Player) {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(Ratrpg.getInstance(), () -> {
